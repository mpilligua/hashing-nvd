...
class ResidualEstimator(nn.Module):
	...
	def forward(self, uv, t):
		# print(uv.shape, t.shape)
		x = torch.hstack((uv, t))
		x = x / 2 + 0.5
		print(x)
		for i, layer in enumerate(self.residual.network):
			# if i == 0:
				# print(x)		
			x = layer(x)
			# if i == 0:
		...
		x = F.softplus(x.to(torch.float32))
		return x
...
class BaseTextureNetwork(nn.Module):
	...
	def forward(self, uv):
		x = uv / 2 + 0.5
		for i in range(len(self.texture.network)):
			x = self.texture.network[i](x)
			# if i == 0:
			# 	print(x)
		x = torch.tanh(x.to(torch.float32))
		x = (x + 1.0) * 0.5
		print(x.shape)
		return x
