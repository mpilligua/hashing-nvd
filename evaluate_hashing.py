...
def eval_data_gen(
    ...
    num_of_maps, texture_size, device, mask_frames):
    '''
    ...
    with torch.no_grad():
        ...
        # batch_jif = jif_all.transpose(0, 1).split(samples_batch, dim=0)
        ...
        for idx in range(len(batch_xyt)):
            ...
            if model_alpha is not None: 
                ...
                alphas.append(batch_alphas[idx].cpu().numpy())
...
def eval_hashing(
    ...
    iteration, optimizer_all, device, save_checkpoint=True, wandb = None, mask_frames = None, save_videos = False):
    ...
    with torch.no_grad():
        ...
        rec_masks, xyts, alphas, uvs, residuals, rgbs = eval_data_gen(model_F_mappings, model_alpha, jif_all, number_of_frames, rez, samples_batch, num_of_maps, texture_size, device, mask_frames=mask_frames)
        # print(residuals)
        # exit(0)
        ...
        wandb.log({"evaluation/PSNR (w/ residual)": psnr,
                    "evaluation/PSNR (w/o residual)": psnr_no_residual})
        ...
        #     }
        #     if model_alpha is not None:
        #         saved_dict['model_F_alpha_state_dict'] = model_alpha.state_dict()
