...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=Path, default=Path('/home/mpilligua/Video-Segmentation/Data/DAVIS/JPEGImages/480p/'), help='folder to process')
    ...
    lst = os.listdir(args.data_path)
    print(len(lst))
    
    for sequence_name in lst:
        print(sequence_name)

        files = sorted((args.data_path + "/" + sequence_name).glob('*.jpg'))
        out_flow_dir = Path("/data/datasets/DAVIS-2017/trainval-480p/DAVIS/flow/" + sequence_name)
